...
namespace Practice_Linq
{
    public class Program
    {
        ...
        static void Query13(List<FootballGame> games)
        {
            ...
            var selectedGames = games.Where(x => x.Neutral == true); // допиши запит
            var topCountries = selectedGames.GroupBy(x => x.Country);
            var newTypeGames = topCountries.Select(x => new
            {
                Country = x.Key,
                Count = x.Count()
            });
            newTypeGames = newTypeGames.OrderByDescending(x => x.Count).Take(3);
        ...
        static void Query14(List<FootballGame> games)
        {
            ...
            var selectedGames = games.GroupBy(x => x.Tournament); // допиши запит
            var topTournirs = selectedGames.Select(x => new
            {
                Tournament = x.Key,
                AvgGoals = x.Average(x => x.Away_score + x.Home_score)
            });
            topTournirs = topTournirs.OrderByDescending(x => x.AvgGoals).Take(5);

            // Результат
            Console.WriteLine("\n======================== QUERY 14 ========================");

            //foreach
            foreach (var item in topTournirs)
            {
                Console.WriteLine($"{item.Tournament} - {item.AvgGoals}");
            }
        }
